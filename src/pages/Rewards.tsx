...
const milestones = [{
  level: 1,
  points: 500,
  title: "Full Stack + Custom GPT",
  status: "completed",
  reward: "Certificate"
}, {
  level: 2,
  points: 1000,
  title: "Effective Prompts + AI Tools",
  status: "completed",
  reward: "Gift Vouchers + Certificate"
}, {
  level: 3,
  points: 2000,
  title: "Automations",
  status: "current",
  reward: "Certificate + Special Perks"
}, {
  level: 4,
  points: 3500,
  title: "Productising AI Solutions",
  status: "locked",
      ...
      <div className="container mx-auto px-6 py-8 space-y-8">
        ...
        <Card className="shadow-medium">
          ...
          <CardContent>
            <div className="space-y-4">
              {milestones.map(milestone => <div key={milestone.level} className={`flex items-center justify-between p-4 rounded-lg border ${milestone.status === "completed" ? "bg-success/10 border-success/20" : milestone.status === "current" ? "bg-primary/10 border-primary/20" : "bg-muted/30 border-muted"}`}>
                  <div className="flex items-center space-x-3">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${milestone.status === "completed" ? "bg-success text-white" : milestone.status === "current" ? "bg-primary text-white" : "bg-muted text-muted-foreground"}`}>
        ...
        <div className="space-y-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {rewards.map(reward => <Card key={reward.id} className={`hover:shadow-medium transition-smooth ${reward.status === "earned" ? "border-success/50" : ""}`}>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div className="text-2xl">{reward.icon}</div>
                    <Badge variant={reward.status === "earned" ? "default" : reward.status === "in-progress" ? "secondary" : "outline"}>
                      {reward.status === "earned" ? "Earned" : reward.status === "in-progress" ? "In Progress" : "Locked"}
                    </Badge>
                  </div>
                ...
                <CardContent className="space-y-4">
                  ...
                  {reward.status === "earned" && <div className="space-y-2">
                      <div className="flex items-center space-x-2 text-sm text-muted-foreground">
                        <Calendar className="h-4 w-4" />
                        <span>Earned on {reward.earnedDate}</span>
                      </div>
                      <div className="flex space-x-2">
                        <Button size="sm" variant="outline" className="flex-1" onClick={() => handleClaimReward(reward.id)}>
                          <Gift className="h-4 w-4 mr-1" />
                          Claim
                        </Button>
                        {reward.type === "voucher" && <Button size="sm" variant="ghost" onClick={() => handleDownloadCertificate(reward.title)}>
                            <Download className="h-4 w-4" />
                          </Button>}
                      </div>
                    </div>}

                  {reward.status === "in-progress" && reward.progress && <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span>Progress</span>
